...
class InputExample(object):
    ...
    def __init__(self, example_id, tokens, triggerL, triggerR, label=None, trigger_word=''):
        ...
        self.label = label
        self.trigger = trigger_word # BCJ added
...
# BCJ        textR = tokenizer.tokenize(" ".join(example.tokens[example.triggerL:example.triggerR]))+['[unused1]']+tokenizer.tokenize(" ".join(example.tokens[example.triggerR:]))
        textR = tokenizer.tokenize(example.trigger)+['[unused1]']+tokenizer.tokenize(" ".join(example.tokens[example.triggerR:]))
        #print('textL', textL)
        #print('trigger', example.trigger)
        #print('textR', textR)
